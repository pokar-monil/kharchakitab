...
export const AnalyzeView = ({
  ...
  useEffect(() => {
    ...
    const loadMetrics = async () => {
      ...
      try {
        ...
        const [baseTransactions, prevTransactions, rangeTransactionsData] =
          await Promise.all([
  ...
  }, [allocationMode, rangeTransactions]);
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="mt-4">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div className="relative flex-1">
              <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[var(--kk-ash)]" />
              <input
                value={query}
                onChange={(event) => setQuery(event.target.value)}
                placeholder="Search expenses..."
                className="kk-input pl-10 text-sm"
              />
            </div>
            <button
              type="button"
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2 text-[var(--kk-ash)]">
            <div className="flex flex-wrap items-center gap-x-3 gap-y-2 rounded-xl border border-[var(--kk-smoke)] bg-white/60 px-3 py-2">
              <label className="kk-label flex items-center gap-2 text-[0.6rem]">
                <span>From</span>
                <input
                  type="date"
                  value={customStart}
                  ...
                  className="kk-input kk-input-compact kk-date-input w-[6rem] bg-transparent normal-case text-[var(--kk-ink)]"
                />
                <button
                  type="button"
                  onClick={() => triggerPicker(customStartRef)}
                  className="kk-icon-btn kk-icon-btn-ghost kk-icon-btn-sm"
                >
                  <Calendar className="h-3 w-3" />
                </button>
              </label>
              <ArrowRight className="h-3 w-3 text-[var(--kk-ash)]" />
              <label className="kk-label flex items-center gap-2 text-[0.6rem]">
                <span>To</span>
                <input
                  type="date"
                ...
                <button
                  type="button"
                  onClick={() => triggerPicker(customEndRef)}
                  className="kk-icon-btn kk-icon-btn-ghost kk-icon-btn-sm"
                >
                  <Calendar className="h-3 w-3" />
                </button>
            ...
            >
                ...
                {isMetricsLoading ? (
                  ...
                  <>
                    <div className="flex items-center gap-2">
                      ...
                      This period: <span className="text-[var(--kk-ink)]">{trendRangeLabels.current || "—"}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="h-2 w-2 rounded-full bg-[var(--kk-ash)]" />
                      Previous: <span className="text-[var(--kk-ink)]">{trendRangeLabels.previous || "—"}</span>
                    </div>
                  </>
                )}
              </div>
              {hasPrevData && (
                <div className="mt-2 text-sm">
                  <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[11px] font-medium ${trendMetrics.delta >= 0 ? "text-[var(--kk-ember-deep)]" : "text-[var(--kk-ash)]"}`}>
            ...
            >
              ...
              <div className="mt-4 flex flex-col items-center gap-4">
                {donutSegments.length >= 1 ? (
                  <svg aria-hidden viewBox="0 0 42 42" className="h-24 w-24">
                    <circle cx="21" cy="21" r="16" fill="none" stroke="var(--kk-smoke-heavy)" strokeWidth="6" />
                  ...
                  <div className="flex h-24 w-24 items-center justify-center rounded-full border border-[var(--kk-smoke-heavy)] text-[var(--kk-ash)]">—</div>
                )}
                <div className="grid gap-2 text-[13px] text-[var(--kk-ash)]">
                  {allocationMetrics.topCategories.map((category, index) => {
                    ...
                    return (
                      <div key={category.label} className="flex items-center gap-3">
                        <span className="h-2.5 w-2.5 rounded-full" style={{ background: donutSegments[index]?.color }} />
                        <span className="text-[var(--kk-ink)]">{category.label}</span>
                        <span className="text-[var(--kk-ash)]">{absoluteValue} ({share}%)</span>
                      </div>
                    );
                  })}
                </div>
      ...
      <motion.div
        ...
        className="kk-card p-5"
      >
        <div className="kk-label mb-4">Transactions</div>
            ...
            {groupedItems.map((group) => (
              <div key={group.key}>
                <div className="mb-3 flex items-center gap-2">
                  <div className="kk-label text-[var(--kk-ember)]">{group.label}</div>
                  <div className="flex-1 border-t border-[var(--kk-smoke)]" />
                  <div className="kk-meta font-medium">₹{formatCurrency(group.total)}</div>
                </div>
                <div className="space-y-2">
                  <AnimatePresence mode="popLayout">
                    {group.items.map((tx, index) => (
                      <TransactionRow
                        key={tx.id || `${group.key}-${index}`}
                        tx={tx}
                        index={index}
                        metaVariant="time"
                        hasEdit={hasEdit}
